...
$is_guru = ($role === 'Guru RPL');
$is_guest = ($role === 'guest'); // Perbaikan huruf kecil

// Pesan modal selamat datang
if (!empty($is_admin) || !empty($is_pelanggan) || !empty($is_guru)) {
    $message = "Selamat datang " .htmlspecialchars($username)." Di X-Kantin";
} else {
    ...
    <div class="modal-dialog">
        <div class="modal-content bg-dark">
            <div class="modal-header text-light">
                <h5 class="modal-title" id="adminModalLabel">Admin Alert</h5>
...
<div class="modal fade" id="guruModal" tabindex="-1" aria-labelledby="guruModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content bg-dark">
            <div class="modal-header text-light">
                <h5 class="modal-title" id="guruModalLabel">Guru Alert</h5>
                <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body text-light text-center">
                <?php echo $message ?>
...
document.addEventListener("DOMContentLoaded", function() {
    <?php if ($is_admin): ?>
        ...
        adminModal.show();
    <?php elseif ($is_guru): ?>
        var guruModal = new bootstrap.Modal(document.getElementById('guruModal'));
        guruModal.show();
    <?php elseif ($is_guest): ?>
